...
|---|---|---|---|---|
| EN-01 | 胞子キノコ | キノコ | 待機・移動・飛びかかり・被弾・死亡（胞子が弾ける） | 動き作成・.anim 書き出し済み |
| EN-02 | 巣守りのバチ | ハチ | 待機・移動・刺す・突進・被弾・死亡 | 動き作成・.anim 書き出し済み |
| EN-03 | 妖精の灯 | 光の妖精（未作成モデル） | 待機・逃げる・眠り粉を撒く・被弾・死亡 | 動き作成・.anim 書き出し済み |
| EN-04 | 輪の茸番 | キノコ | キノコ共通＋踏みつけ（硬直0.9秒） | 動き作成・.anim 書き出し済み |
| EN-16 | 洞窟茸 | キノコ | キノコ共通 | 動き作成・.anim 書き出し済み |
| EN-17 | 岩ダンゴ | 岩の子 | 待機・移動・転がり・被弾・死亡 | 動き作成・.anim 書き出し済み |
| EN-42 | 水路ナメクジ | ナメクジ（未作成モデル） | 待機・這う・噛みつき・被弾・死亡 | 動き作成・.anim 書き出し済み |
| EN-43 | 苔キノコ | キノコ | キノコ共通 | 動き作成・.anim 書き出し済み |
...
- `tools/export_unity.mjs` を作成し、28 本の .anim を `unity/<体>/` に書き出した（合計約 8.5 MB）
  ...
  - 次：`unity/EnemyMotion.cs`（再生・フェード・イベント）→ README → zip 納品
