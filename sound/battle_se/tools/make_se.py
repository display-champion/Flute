...
def blade_swing(d, f_lo, f_hi, peak, width, ring=0.0, ring_f=5200):
    w = whoosh(d, [(0, f_lo), (peak, f_hi), (d, f_lo * 0.8)], peak, width, skew=1.6, q=1.6, body=0.25)
    x = w
    if ring:
        r = metal_ring(d, ring_f, tau=0.12) * bell_env(d, peak, width * 0.7, 1.5)
        x = x + ring * norm(r)
    return x
...
def slash_hit(d, weight):
    ...
    ring = metal_ring(d, 4300 - 800 * weight, tau=0.08 + 0.1 * weight)
    x = mix(d, [(0, click(0.004, 3000), 0.7), (0, cut, 0.8), (0.002, body, 0.8 + 0.5 * weight),
                (0.002, flesh, 0.5), (0, ring, 0.18)])
...
def _():
    d = 0.22
    return mix(d, [(0, burst(d, 2000, 9000, 0.025, 0), 0.8), (0, thump(d, 140, 70, 0.04, 2), 0.7), (0, metal_ring(d, 4800, 0.06), 0.15)])
...
@se("Fina_Rush_Swing", "フィナ（剣）", SWING, "時間停止中の高速連撃（鋭く短い剣閃。きらめき入り）", "時間停止中の□（A・B の振りごと）")
def _():
    d = 0.25
    w = blade_swing(d, 1400, 6500, 0.07, 0.025, ring=0.2, ring_f=6200)
    return w + 0.25 * norm(sparkle(d, 6, 5000, 11000, (0.02, 0.05)))


@se("Fina_Warp", "フィナ（剣）", LIGHT, "時間停止中のワープ（吸い込まれて、目の前にパッと現れる）", "時間停止中、最初の□のワープ")
def _():
    d = 0.45
    f = curve(d, [(0, 300), (0.28, 3200), (d, 3200)], "exp")
    zip_ = osc(f, d, "saw") * env(d, [(0, 0), (0.05, 0.4), (0.28, 1), (0.3, 0), (d, 0)])
    zip_ = bp(zip_, f * 1.5, 1.5)
    air = whoosh(d, [(0, 500), (0.28, 5000), (d, 2000)], 0.26, 0.06, skew=0.6)
    pop = mix(d, [(0.29, burst(0.12, 800, 8000, 0.02, 0), 1.0), (0.29, thump(0.12, 300, 120, 0.02), 0.5)])
    x = mix(d, [(0, zip_, 0.5), (0, air, 0.7), (0, pop, 0.9), (0.29, sparkle(0.16, 10, 4000, 10000), 0.3)])
    return reverb(x, 0.5, 0.2)


@se("JustDodge_TimeStop", "フィナ（剣）", HEAVY, "ジャスト回避→時間停止（世界が伸びて止まる「ヴゥン…」と、時計の音）", "ジャスト回避の成功")
def _():
    d = 2.2
    # 逆再生のシンバル風に吸い込み → 低く沈む音 → 止まった世界の時計
    swell = rev(norm(hp(white(0.5), 3000)) * expdec(0.5, 0.15))
    f = curve(d, [(0, 900), (0.5, 900), (1.2, 70), (d, 60)], "exp")
    dive = (osc(f, d, "sine") + 0.4 * osc(f * 1.5, d, "sine") + 0.25 * osc(f * 0.5, d, "tri")) * env(d, [(0, 0), (0.45, 0), (0.5, 1), (1.6, 0.5), (d, 0)])
    boom = thump(1.2, 80, 30, 0.35, 2)
    ticks = mix(d, [(0.9 + 0.3 * i, metal_ring(0.2, 2400 if i % 2 else 1800, 0.05), 0.35 * (0.8 ** i)) for i in range(4)])
    x = mix(d, [(0, swell, 0.8), (0, dive, 0.7), (0.5, boom, 1.0), (0.5, burst(0.4, 200, 6000, 0.08, -3), 0.5), (0, ticks, 1.0)])
    return reverb(x, 1.8, 0.35, lpf=4000)


@se("TimeStop_End", "フィナ（剣）", LIGHT, "時間停止が解ける（止まった世界が戻る、上がっていく音とガラスの響き）", "時間停止の終わり（5秒）")
def _():
    d = 1.2
    f = curve(d, [(0, 60), (0.5, 900), (d, 900)], "exp")
    rise = (osc(f, d) + 0.4 * osc(f * 1.5, d)) * env(d, [(0, 0), (0.1, 0.6), (0.5, 1), (0.55, 0), (d, 0)])
    rel = mix(d, [(0.5, burst(0.5, 1500, 10000, 0.1, 0), 0.7), (0.5, partials(0.7, [2100, 3150, 4700, 6300], [0.4, 0.3, 0.2, 0.15]), 0.4)])
    return reverb(rise * 0.6 + rel, 1.2, 0.3)
...
def _():
    ...
    return mix(d, [(0, catch, 0.7), (0.05, thump(0.2, 180, 80, 0.04), 0.5), (0.08, up, 0.8), (0.1, sparkle(0.6, 10, 3000, 8000), 0.12)])
...
def _():
    ...
    pierce = mix(0.6, [(0, burst(0.6, 1500, 9000, 0.04, 0), 0.8), (0, burst(0.6, 100, 1500, 0.12, -4), 0.9),
                       (0, thump(0.6, 120, 40, 0.15, 3), 1.0), (0, metal_ring(0.6, 3300, 0.35), 0.3)])
...
def _():
    ...
    x = mix(d, [(0, slash_hit(0.35, 0.6), 0.8), (0.05, thump(0.4, 90, 40, 0.12), 0.6),
                (0.08, norm(bp(white(0.6), curve(0.6, [(0, 3000), (0.6, 800)], "exp"), 2)) * expdec(0.6, 0.2), 0.3),
                (0.1, sparkle(0.5, 8, 2500, 7000), 0.15)])
...
@se("Enemy_Telegraph", "敵の攻撃", WARN, "攻撃の予告（赤い輪郭と同時に鳴る、鋭い「キィン」）", "敵の攻撃の0.3秒前（無音の影 EN-38 だけは鳴らさない）")
def _():
    d = 0.5
    x = partials(d, [1760, 1760 * 2.01, 1760 * 2.93, 1760 * 4.1], [0.18, 0.12, 0.08, 0.05], [1, 0.5, 0.3, 0.2])
    x = x + 0.6 * np.roll(partials(d, [2350, 4700], [0.14, 0.08], [1, 0.4]), N(0.06))
    return reverb(x * env(d, [(0, 0), (0.003, 1), (d, 1)]), 0.5, 0.15)
...
def _():
    ...
    parts = []
    for i in range(3):
        s = hp(white(0.06), 2500 - 300 * i) * env(0.06, [(0, 0), (0.004, 1), (0.06, 0)]) * (1 + 0.5 * np.sin(phase_of(180 + 40 * i, 0.06)))
...
def _():
    ...
    hit = mix(0.9, [(0, thump(0.9, 80, 28, 0.22, 3.5), 1.0), (0, burst(0.5, 200, 4000, 0.06, -2), 0.8),
                    (0, metal_ring(0.8, 900, 0.3), 0.2), (0.01, grains(0.6, 40, freq=(400, 4000)), 0.3)])
    return reverb(mix(d, [(0, swing, 0.5), (0.4, hit, 1.0)]), 1.0, 0.18, lpf=3000)


@se("Enemy_SwordSlash", "敵の攻撃", HIT, "敵の剣の斬撃（長剣の重い風切りと刃鳴り）", "聖堂騎士の剣の2連撃（1撃ごと）")
def _():
    d = 0.6
    sw = blade_swing(0.4, 500, 2800, 0.15, 0.06, ring=0.3, ring_f=2900)
...
def _():
    ...
    knock = mix(0.3, [(0, partials(0.3, [420, 1090, 1830, 2600], [0.05, 0.03, 0.02, 0.015]), 1.0), (0, thump(0.3, 180, 90, 0.04), 0.8), (0, burst(0.3, 500, 3000, 0.02), 0.5)])
    return mix(d, [(0, sw, 0.4), (0.12, knock, 1.0)])


@se("Enemy_Scissors", "敵の攻撃", HIT, "大きな鋏がはさみ込む（刃がすれ合い「シャキン」と閉じる）", "刈り込み鋏のはさみ込み（正面2連、1回ごと）")
def _():
    d = 0.5
    scrape = hp(white(0.18), 3000) * env(0.18, [(0, 0), (0.1, 0.6), (0.18, 1)]) * (1 + 0.4 * np.sin(phase_of(60, 0.18)))
    snap = mix(0.4, [(0, click(0.004, 2000), 1.0), (0, metal_ring(0.4, 3700, 0.2), 0.5), (0, metal_ring(0.4, 5100, 0.12), 0.3), (0, thump(0.2, 300, 150, 0.02), 0.4)])
    return mix(d, [(0, norm(scrape), 0.4), (0.17, snap, 1.0)])


@se("Enemy_Chain", "敵の攻撃", HIT - 1, "鎖を伸ばして打つ（じゃらじゃら→「ジャラン」）", "鎖の魂の鎖")
def _():
    d = 0.8
    rattle = np.zeros(N(0.45))
    for i in range(18):
        at = N(0.02 * i + RNG.uniform(0, 0.01))
        r = metal_ring(0.1, RNG.uniform(2500, 5000), 0.03) * RNG.uniform(0.3, 0.8)
        rattle[at:at + len(r)] += r[: len(rattle) - at]
    hit = mix(0.5, [(0, metal_ring(0.5, 1900, 0.2), 0.6), (0, metal_ring(0.5, 2700, 0.15), 0.5), (0, burst(0.3, 1000, 6000, 0.03), 0.6), (0, thump(0.3, 200, 90, 0.04), 0.4)])
    return reverb(mix(d, [(0, rattle, 0.5), (0, whoosh(0.4, [(0, 500), (0.35, 2500)], 0.33, 0.08, 0.4), 0.3), (0.38, hit, 1.0)]), 0.8, 0.2)
...
@se("LightArrow_Shoot", "敵の攻撃", SHOT, "光の矢を放つ（澄んだ音が伸びて飛んでいく）", "光の番人の光の矢")
def _():
    d = 0.8
    f = curve(d, [(0, 1400), (0.5, 2600), (d, 2600)], "exp")
    tone = (osc(f, d) + 0.4 * osc(f * 2, d) + 0.2 * osc(f * 3.01, d)) * env(d, [(0, 0), (0.01, 1), (0.6, 0.3), (d, 0)])
    return reverb(mix(d, [(0, tone, 0.5), (0, whoosh(0.5, [(0, 3000), (0.5, 7000)], 0.05, 0.12, 2.5, q=2, body=0), 0.5), (0, sparkle(0.6, 14, 4000, 10000), 0.25)]), 0.8, 0.25)
...
@se("Sleep_Powder", "敵の攻撃", LIGHT, "眠り粉を撒く（ふわっと舞う柔らかいきらめき）", "妖精の灯の眠り粉（移動が2秒半分）")
def _():
    d = 1.2
    puff = lp(white(0.4), 1800) * env(0.4, [(0, 0), (0.08, 1), (0.4, 0)])
    sp = sparkle(1.0, 30, 2500, 7000, (0.05, 0.2))
    return reverb(mix(d, [(0, norm(puff), 0.5), (0.05, sp * env(1.0, [(0, 1), (1.0, 0.2)]), 0.4)]), 1.0, 0.3)
...
def _():
    ...
    return mix(d, [(0, pop, 1.0), (0.02, norm(drift), 0.3), (0.05, sparkle(0.5, 8, 1500, 4000), 0.1)])


@se("Steam_Blast", "敵の攻撃", LOOP - 1, "熱い湯気を吹きかける（「シュウゥ」と長く噴き出す、約1.5秒）", "湯煙の精の湯気（3秒で計12）")
def _():
    d = 1.6
    hiss = bp(white(d), curve(d, [(0, 2500), (0.2, 4500), (d, 3000)], "exp"), 0.6) * env(d, [(0, 0), (0.08, 1), (1.3, 0.8), (d, 0)])
    rum = lp(pink(d), 400) * env(d, [(0, 0), (0.1, 1), (d, 0)])
    return mix(d, [(0, norm(hiss), 0.8), (0, norm(rum), 0.3), (0.05, bubbles(0.6, 10, 300, 900), 0.15)])
...
def _():
    ...
    x = mix(d, [(0, norm(chord) * e, 0.6), (0, norm(roar) * e, 0.8), (0, explosion(0.8, 0.7), 0.6), (0.05, sparkle(1.2, 30, 3000, 9000), 0.12)])
...
@se("Magic_Circle_Appear", "魔法", LIGHT, "魔法陣が現れる（光が集まって輪が広がる、澄んだ和音）", "魔法陣クリップの Appear")
def _():
    d = 1.3
    notes = [523, 659, 784, 1047, 1319]
    parts = [(0.06 * i, partials(1.0, [f, f * 2, f * 3.01], [0.5, 0.25, 0.12], [1, 0.3, 0.1]), 0.5) for i, f in enumerate(notes)]
    parts.append((0, whoosh(0.5, [(0, 400), (0.3, 3000)], 0.28, 0.1, 0.6), 0.4))
    parts.append((0.1, sparkle(0.9, 18, 3000, 9000), 0.2))
    return reverb(mix(d, parts), 1.2, 0.3)


@se("Magic_Charge", "魔法", LIGHT, "魔力を溜める（うなりが高まっていく、約0.6秒）", "魔法陣クリップの Charge")
def _():
    d = 0.8
    f = curve(d, [(0, 150), (0.6, 600), (d, 650)], "exp")
    ...
    return reverb(mix(d, [(0, norm(tone), 0.7), (0, sparkle(0.6, 14, 3000, 8000) * env(0.6, [(0, 0.2), (0.6, 1)]), 0.25),
...
def magic_shot(e):
    ...
    if e == "Light":
        chord = partials(0.7, [1319, 1760, 2637, 3520], [0.3, 0.25, 0.2, 0.15])
        f = curve(0.4, [(0, 1500), (0.4, 4000)], "exp")
        return reverb(mix(d, [(0, chord, 0.5), (0, osc(f, 0.4) * expdec(0.4, 0.12), 0.3), (0, sparkle(0.6, 25, 4000, 11000), 0.3), (0, shot_whoosh(0.6, 2000, 6000), 0.6)]), 0.9, 0.3)
    if e == "Dark":
...
def magic_impact(e):
    ...
    if e == "Light":
        chord = partials(1.1, [1047, 1319, 1568, 2093, 2637, 3136], [0.6, 0.5, 0.45, 0.4, 0.3, 0.25])
        flash = burst(0.5, 1500, 12000, 0.1, 0)
        return reverb(mix(d, [(0, flash, 0.7), (0, chord, 0.5), (0, thump(0.3, 200, 90, 0.05), 0.4), (0.02, sparkle(1.0, 40, 3000, 11000), 0.3)]), 1.3, 0.3)
    if e == "Dark":
...
SHOT_DESC = dict(Fire="「ボッ」と火がついて燃えながら飛ぶ", Ice="澄んだガラスのような響きと冷たいきらめき", Thunder="「バチッ」とはじける電気のうなり",
                 ...
                 Light="澄んだ和音がきらめきながら飛ぶ", Dark="低くうなる不気味な音")
IMPACT_DESC = dict(Fire="爆発してパチパチ燃え残る", Ice="氷が砕け散る", Thunder="落雷の「バリッ」と遠雷", Wind="突風が吹き抜ける",
                   Water="水しぶきがはじけて泡立つ", Earth="岩が砕けて破片が散らばる", Light="まぶしくはじけて鐘のように響く",
...
def _():
    ...
    x = mix(d, [(0, thump(0.8, 90, 35, 0.2, 3), 1.0), (0, up, 0.8), (0.05, partials(1.2, [262, 392, 523, 784], [0.6, 0.5, 0.4, 0.3]), 0.25), (0.1, sparkle(1.0, 25, 3000, 9000), 0.15)])
