...
def burst(d, lo, hi, tau, color=-3):
    """雑音のかたまり（打撃の「バシッ」）"""
    # 形をつけてから帯域を絞る（立ち上がりが全部の高さに広がる「チッ」を出さない）
    x = band(colored(d, color) * expdec(d, tau, attack=0.0015), lo, hi)
    return norm(x)
...
def grains(d, count, dur=(0.004, 0.03), freq=(800, 5000), density="decay", amp=(0.3, 1.0)):
    ...
    for _ in range(count):
        ...
        g = bp(white(gd) * expdec(gd, gd / 3, attack=0.001), f, 3, stages=2)   # 形をつけてから絞る（立ち上がりの「チッ」を出さない）
...
# ---------------- 材質の音（正弦波の「ドッ」や泡の「ポコッ」を使わない部品） ----------------

def excite(d, length=0.003, bright=4000):
    """叩いた瞬間のこすれ（短い雑音）"""
    e = np.zeros(N(d))
    n = N(length)
    e[:n] = lp(RNG.standard_normal(n), bright) * np.linspace(1, 0, n) ** 2
    return e


def modal(d, freqs, decays, amps=None, length=0.003, bright=4000):
    """雑音で叩いた共鳴（木・石・鉄の「コッ」「ゴッ」）。音の高さは動かない。decays は各共鳴の減衰（秒）"""
    e = excite(d, length, bright)
    out = np.zeros(N(d))
    amps = amps or [1.0] * len(freqs)
    for f, tau, a in zip(freqs, decays, amps):
        q = max(0.7, np.pi * f * tau)
        y = bp(e, f, q)
        out += a * y / (np.max(np.abs(y)) + 1e-12)
    # 叩いた瞬間の高い「カチッ」を丸める（いちばん高い共鳴の 3 倍より上を落とす）
    return lp(out, min(max(freqs) * 3, 6000), stages=2)


def body(d, fc, tau, attack=0.002, color=-6):
    """重さのある鈍い「ドサッ」（音程のない低い雑音）"""
    x = lp(colored(d, color), fc, stages=2)
    return norm(x) * expdec(d, tau, attack=attack)


def droplets(d, count, fmin=700, fmax=2400):
    """しずく（音程が動かない短い「ピチャ」）"""
    out = np.zeros(N(d))
    for _ in range(count):
        at = RNG.uniform(0, d * 0.9)
        gd = RNG.uniform(0.01, 0.03)
        f = np.exp(RNG.uniform(np.log(fmin), np.log(fmax)))
        g = bp(white(gd) * expdec(gd, gd / 4, attack=0.002), f, 6, stages=2)
        g = norm(g) * RNG.uniform(0.2, 1)
        s = N(at)
        e = min(len(out), s + len(g))
        out[s:e] += g[: e - s]
    return out
...
def gurgle(d, lo=250, hi=700, rate=18):
    """水や泥がうねる（中心の高さがゆらぐ雑音）"""
    fc = np.exp(np.log(lo) + (np.log(hi) - np.log(lo)) * (0.5 + 0.5 * slow_noise(d, rate)))
    return norm(bp(pink(d), fc, 2.5) * (0.6 + 0.4 * slow_noise(d, rate * 1.7)))
